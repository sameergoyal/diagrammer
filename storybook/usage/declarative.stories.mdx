...
Diagrammer exposes several declarative DOM APIs that allow you to just add a few data attributes to the DOM content you render and then avoid having to detect user interaction and fire actions into diagrammer.
...
<div
  ...
  data-draggable={true}
  data-type={DiagrammerComponents.POTENTIAL_NODE}
  data-id={typeId}
>
  {nodeText}
</div>;
...
In case the default connectors rendered by diagrammer don't suffice, you can render your own connectors. You just need to add some data attributes, to make it function like a connector for a node. For example, to make an input connector, just add these data attributes:
...
<div>
  <div
    data-event-target="true"
    data-dropzone="true"
    data-type={DiagrammerComponents.NODE_CONNECTOR}
    data-id={nodeId}
  >
...
<div>
  <div
    data-event-target="true"
    data-draggable="true"
    data-type={DiagrammerComponents.PANEL_DRAG_HANDLE}
    data-id={panelId}
  >
    {customDragHandle}
  </div>
...
<div>
  Node Content
  <div data-event-target="true" data-draggable="true">
