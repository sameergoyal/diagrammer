...
## Diagrammer Instance APIs

There are a few APIs that are accessed directly from the diagrammer object instance.

### Update Container

This API should be called by the consumer whenever there is a change to the size of the container that diagrammer is rendering in(Referred to by the first argument of diagrammer constructor). When diagrammer is rendered full screen, this should also be called when the window resizes.
...
This API should be called by the consumer when the container is about to be destroyed. This usually happens when the user navigates away from a page that uses diagrammer to a new page in the application.
...
Usually you will create a React/Preact Component wrapper that will instantiate Diagrammer. When this component unmounts due to a route change or any other reason, it will destroy diagrammer before it unmounts.
...
## Diagrammer API Object

Most functionality is exposed using the API object accessed on the diagrammer instance's `api` key. This object contains APIs that facilitate the manipulation of the diagrammer state such that you dont need to fire actions to achieve the same effect.
...
**Note**: Please note that the workflow layout uses the library Dagre which is not included within the Diagrammer package and not marked as a peer dependency. This is because not all consumers of this library use the workflow based layout API. Using this without importing Dagre will cause client side errors in your application. For using this layout, please installe dagre into your application like so:
`npm install dagre`
...
```javascript
diagrammer.api.setEditorMode(EditorMode.SELECT);
```
...
```javascript
diagrammer.api.focusNode("myAwesomeNodeId");
```
...
diagrammer.api.focusNode("myAwesomeNodeId", 50, 400);
```
...
```javascript
// Left panel is 50px wide & right panel is 400px wide & both are fixed
diagrammer.api.focusSelected(50, 400);
```
...
```javascript
// Left panel is 50px wide & right panel is 400px wide & both are fixed
diagrammer.api.fit(50, 400, ["myAwesomeNode1", "myAwesomeNode2"]);
```
...
diagrammer.api.zoomOut();
// You can specify the zoom
diagrammer.api.zoomOut(100);
```
...
```javascript
// Default zoom is 50 units
diagrammer.api.zoomIn();
// You can specify the zoom
diagrammer.api.zoomIn(100);
```
