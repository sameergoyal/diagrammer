...
Your plugin may or may not want to specify some behavior. Some plugins, for example, could choose to render additional UI based on current behavior like the [minimap plugin](https://github.com/sameergoyal/diagrammer-plugin-minimap)), but in some cases plugins might want to override behavior. In those cases, the plugin have 2 ways to override behavior. In those cases, the plugin
...
**Note**: The above approach has a shortcoming that consumer then has to choose between using this plugin and using their own reducer. To solve this problem, diagrammer exposes a utility called `sequenceReducers`.
...
const diagrammer = new Diagrammer(
  ...{
    consumerRootReducer: sequenceReducers(
      testPluginReducer,
      consumerRootReducer
    ),
  }
...
```javascript
const testPluginReducer = (state: DiagrammerState<NodeType, EdgeType>, action: AnyAction) => {
  switch (action.type) {
